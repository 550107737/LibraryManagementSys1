    ...
    <script>
        $(document).ready(function () {
			...
			$("#table_list").bootstrapTable({
			    ...
			    columns: [{
			        ...
			        field: "empty",
                    formatter: function (value, row, index) {
                        var operateHtml='<button class="btn btn-primary btn-xs" type="button" onclick="edit(\''+row.checkId+'\')"><i class="fa fa-edit"></i>&nbsp;人工盘点</button> &nbsp;';
                        return operateHtml;
                    }
			    }]
			});
        ...
        function edit(id){
        	layer.open({
        	      ...
        	      shade: false,
        	      area: ['893px', '600px'],
        	      content: '${ctx!}/booksCheckCtrl/edit/' + id,
        	      end: function(index){
        ...
        function edit(id){
            layer.confirm('确定盘点完成吗?', {icon: 3, title:'提示'}, function(index){
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "${ctx!}/booksCheckCtrl/edit/" + id,
