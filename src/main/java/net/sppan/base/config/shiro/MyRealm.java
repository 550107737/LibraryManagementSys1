...
//		authorizationInfo.setStringPermissions(shiroPermissions);
		...
		SimpleAuthorizationInfo authorizationInfo = new SimpleAuthorizationInfo();
		UserModel dbUser = userService2.findByUserId(userModel.getUserId());
		Set<String> shiroPermissions = new HashSet<>();
		Set<String> roleSet = new HashSet<String>();
		RoleModel role=roleService.findById(dbUser.getUserSession());
...
//		SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(user, password, getName());
		...
		UserModel userModel = userService2.findByUserId(userId);
		String password = new String((char[]) token.getCredentials());
		if (userModel == null) {
			throw new UnknownAccountException("账号或密码不正确");
		}
		if (!MD5Utils.md5(password).equals(userModel.getUserPassword())) {
			throw new IncorrectCredentialsException("账号或密码不正确");
		}
