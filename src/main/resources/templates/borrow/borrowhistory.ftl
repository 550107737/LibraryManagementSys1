                    ...
                    <div class="ibox-content">
						<p>
							<@shiro.hasPermission name="system:borrow:bothadmin">
                                <button class="btn btn-success " type="button" onclick="overdueSearch();"><i class="fa fa-plus"></i>&nbsp;用户逾期记录查询</button>
							</@shiro.hasPermission>
						</p>
                        <hr>
    ...
    <script>
        $(document).ready(function () {
			...
			$("#table_list").bootstrapTable({
			    ...
			    //获取数据的Servlet地址
			    url: "${ctx!}/borrowHistoryCtrl/list123",
			    //表格显示条纹
			    striped: true,
			    //启动分页
			    pagination: true,
        ...
        function edit(id){
        	layer.open({
        	      ...
        	      shade: false,
        	      area: ['893px', '600px'],
        	      content: '${ctx!}/borrowHistoryCtrl/edit/' + id,
        	      end: function(index){
        ...
        function del(id){
        	layer.confirm('确定删除吗?', {icon: 3, title:'提示'}, function(index){
        		$.ajax({
    	    		   type: "POST",
    	    		   dataType: "json",
					   url: "${ctx!}/borrowHistoryCtrl/delete/" + id,
        ...
        function overdueSearch() {
            layer.open({
                ...
                shade: false,
                area: ['893px', '600px'],
                content: '${ctx!}/borrowHistoryCtrl/overdueList',
                end: function(index){
                    $('#table_list').bootstrapTable("refresh");
                }
            });
